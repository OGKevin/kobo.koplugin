...
**Important Limitation**: When syncing to Kobo, position is rounded to chapter boundaries. When Kobo
syncs back to KOReader, KOReader opens at the exact percentage from Kobo.
...
This diagram shows the complete flow of how books and reading progress move through the system when
using Komga or Calibre Web with the Kobo Plugin. The key points are:
